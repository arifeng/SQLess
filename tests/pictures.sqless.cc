...
bool SQLessDB_db::exec(const std::string& sql_stmt) {
    use();
...
bool SQLessDB_db::has_table_tb() {
    std::string res, sql = "SELECT name FROM (SELECT * FROM sqlite_master UNION ALL SELECT * FROM sqlite_temp_master) WHERE type = \"table\" AND name = \"tb\";";
    return conn_->exec(sql, &res) && res == "tb";
}
...
bool SQLessTable_tb::create() {
    return db_->exec("CREATE TABLE IF NOT EXISTS tb (id INTEGER PRIMARY_KEY AUTO_INCREMENT NOT NULL, path TEXT NOT NULL, type TEXT DEFAULT 'unknown', size BIGINT DEFAULT 0, md5 TEXT, data BLOB); ");
}

bool SQLessTable_tb::drop() {
    return db_->exec("DROP TABLE IF EXISTS \"tb\";");
}

int SQLessTable_tb::row_count(const std::string& condition) {
    std::string res, sql = "SELECT COUNT(*) FROM tb";
...
bool SQLessTable_tb::SelectResult::getRow() {
    ...
    for (int i = 0; i < _columns; i++) {
        if (id) {
            ...
            const char* p = (const char*)sqlite3_column_text(stmt_, i); path_ .assign(p, sqlite3_column_bytes(stmt_, i));
            path = false;
        } else if (type) {
            const char* p = (const char*)sqlite3_column_text(stmt_, i); type_ .assign(p, sqlite3_column_bytes(stmt_, i));
            type = false;
        } else if (size) {
            size_ = sqlite3_column_int64(stmt_, i);
            size = false;
        } else if (md5) {
            const char* p = (const char*)sqlite3_column_text(stmt_, i); md5_ .assign(p, sqlite3_column_bytes(stmt_, i));
            md5 = false;
        } else if (data) {
            const char* p = (const char*)sqlite3_column_text(stmt_, i); data_ .assign(p, sqlite3_column_bytes(stmt_, i));
            data = false;
        } 
    }
...
bool SQLessColumn_tb_id::exists() {
    return table_->database()->exec("SELECT id FROM tb LIMIT 0;");
}

bool SQLessColumn_tb_id::create() {
    return table_->database()->exec("ALTER TABLE tb ADD id INTEGER PRIMARY_KEY AUTO_INCREMENT NOT NULL;");
}
...
bool SQLessColumn_tb_path::exists() {
    return table_->database()->exec("SELECT path FROM tb LIMIT 0;");
}

bool SQLessColumn_tb_path::create() {
    return table_->database()->exec("ALTER TABLE tb ADD path TEXT NOT NULL;");
}
...
bool SQLessColumn_tb_type::exists() {
    return table_->database()->exec("SELECT type FROM tb LIMIT 0;");
}

bool SQLessColumn_tb_type::create() {
    return table_->database()->exec("ALTER TABLE tb ADD type TEXT DEFAULT 'unknown';");
}
...
bool SQLessColumn_tb_size::exists() {
    return table_->database()->exec("SELECT size FROM tb LIMIT 0;");
}

bool SQLessColumn_tb_size::create() {
    return table_->database()->exec("ALTER TABLE tb ADD size BIGINT DEFAULT 0;");
}
...
bool SQLessColumn_tb_md5::exists() {
    return table_->database()->exec("SELECT md5 FROM tb LIMIT 0;");
}

bool SQLessColumn_tb_md5::create() {
    return table_->database()->exec("ALTER TABLE tb ADD md5 TEXT;");
}
...
bool SQLessColumn_tb_data::exists() {
    return table_->database()->exec("SELECT data FROM tb LIMIT 0;");
}

bool SQLessColumn_tb_data::create() {
    return table_->database()->exec("ALTER TABLE tb ADD data BLOB;");
}
